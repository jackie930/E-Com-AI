...
def process_brat(ann_file):
    ...
    df_aspect_att_re = pd.merge(df_aspect_att, df_relations, left_on='id_x', right_on='aspect', how='left')
    df_res = pd.merge(df_aspect_att_re, df_aspect_att_re, left_on='opionion', right_on='id_x', how='left')
    print("total aspect number: ", len(df_res[df_res['type_x_x'] != 'opinion']))
    print("total aspect number with opinion: ", len(df_res[df_res['type_x_y'] == 'opinion']))
    return df_res
...
def process_single_file(text_file,ann_file,task):
    ...
    # add back sent_num
    df_res['start_idx'] = df_res['span_x'].map(lambda x: x[0][0])
    df_res['sent_number'] = df_res['start_idx'].map(lambda x: judge_sen_number(x, df_data))

    if task == 'absa':
        # only keep the labels with (aspect, category, opinion, sentiment)
        df_res_four_labels = df_res[df_res['type_x_y'] == 'opinion']
        df_res_four_labels = df_res_four_labels[-df_res_four_labels['values_y'].isnull()]
        df_res_four_labels['label'] = df_res_four_labels.apply(lambda row: format_absa(row['text_x'], row['values_x'], row['text_y'], row['values_y']), axis=1)
    if task =='aspect_category':
        df_res_four_labels = df_res[df_res['type_y_x']=='category']
        df_res_four_labels['label'] = df_res_four_labels.apply(
            lambda row: format_aspect_category(row['text_x'], row['values_x']), axis=1)
