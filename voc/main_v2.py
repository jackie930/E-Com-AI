...
from eval_utils_v2 import compute_scores,compute_scores_jj
...
def init_args():
    ...
    parser.add_argument("--do_batch_predict", action='store_true', help="Whether to run batch prediction")
    ...
    parser.add_argument("--ckpoint_path", default='./outputs/tasd-cn/ctrip/extraction/cktepoch=1.ckpt', type=str,
                        required=False)
...
#     device='cpu'
    ...
    if custom:
        #jijia
        print ("customer score compute jj")
        raw_scores, res_tag, all_labels, all_predictions = compute_scores_jj(outputs, targets, sents, paradigm,task)
        results = {'raw_scores': raw_scores,
                   'labels': all_labels,
                   "res_tag": res_tag,
                   'preds': all_predictions}
#         results_pred = {'sent':sents,"label":all_labels_with_tag,"pred":all_predictions_with_tag}
        print(f"{args.output_dir}/results-{args.task}-{args.dataset}-{args.paradigm}-allres.pickle")
...
if args.do_train:
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filepath=args.output_dir, prefix="ckt", monitor='loss', mode='min', save_top_k=15
    )
...
if args.do_direct_eval:
    ...
    test_dataset = ABSADataset(tokenizer, data_dir=args.dataset, data_type='test',
                               paradigm=args.paradigm, task=args.task, max_len=args.max_seq_length)
    test_loader = DataLoader(test_dataset, batch_size=args.eval_batch_size, num_workers=0)
    print("<<<< start evaluate")
    raw_scores = evaluate(test_loader, model, args.paradigm, args.task,  sents, custom=False)
    # print(scores)
