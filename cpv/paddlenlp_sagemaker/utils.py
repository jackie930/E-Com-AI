...
def get_relation_type_dict(relation_data):
    ...
    for i in range(len(relation_data)):
        ...
        if relation_data[i][0] not in added_list:
            ...
            if not added:
                added_list.append(relation_data[i][0])
                suffix = relation_data[i][0].rsplit("'s", 1)[1]
    ...
    with tqdm(total=len(raw_examples)) as pbar:
        for line in raw_examples:
            ...
            for entity in entities:
                ...
                entity_cls_prompt_prefix = entity_name + "'s" + prompt_prefix
            ...
            for relation in relations:
                ...
                # The relation prompt is constructed as follows:
                # subject + "的" + predicate
                prompt = entity_map[subject_id]["name"] + "'s" + predicate
                    ...
                    if len(subject_goldens[i]) != 0:
                        ...
                        redundants3 = [
                            subject_goldens[i][random.randrange(
                                len(subject_goldens[i]))] + "'s" + non_ent_label
